...
export async function GET(
  ...
  try {
    ...
    return NextResponse.json(
      { error: "Не вдалося отримати подію" },
      { status: 500 }
    );
  }
}
...
export async function PUT(
  ...
  const limitKey = `events:update:${getClientKey(req)}`;
  ...
  try {
    ...
    const payload = await ensureCoordinates(clampLocation(parsed.data));
...
export async function DELETE(
  ...
  const limitKey = `events:delete:${getClientKey(req)}`;
